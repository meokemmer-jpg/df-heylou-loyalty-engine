"""Audit-Logger fuer DF-HeyLou-Loyalty-Engine [CRUX-MK]."""
...
class AuditLogger:
    def __init__(self, audit_path: Optional[Path] = None, secret: Optional[str] = None):
        if audit_path is None:
            audit_path = Path.home() / ".df-state" / "df-heylou-loyalty-audit.jsonl"
        audit_path.parent.mkdir(parents=True, exist_ok=True)
        self.audit_path = audit_path
        self.secret = secret or os.environ.get(
            "DF_HEYLOU_LOYALTY_AUDIT_SECRET", "skeleton-default-secret"
        )
